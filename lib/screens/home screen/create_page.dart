...
class _CreatePageState extends State<CreatePage> {
  ...
  Future<void> _createBook() async {
    ...
    try {
      ...
      // Prepare files list with correct type handling [[10]]
      List<http.MultipartFile> files = [];
      if (_bookCover != null) {
        files.add(
          await http.MultipartFile.fromPath(
            'book_cover', // Must match PocketBase field name [[5]]
            _bookCover!.path,
          ),
        );
      }

      // Combined create + upload operation [[8]]
      final book = await userService.pb.collection('books').create(
        ...
        files: files, // Always non-null list [[10]]
